...
class DockerRealm:
    def requestAvatar(self, username, mind, *interfaces):
        user = DockerVolumeConchUser(username)
...
class DockerVolumeConchUser(ConchUser):
    ...
    def __init__(self, container_name):
        ...
        self.volumes = container_database[container_name]["Volumes"]
    ...
    def logout(self):
        ...
        for listener in self.listeners.itervalues():
            self._runAsUser(listener.stopListening)
        log.msg('avatar %s logging out (%i)' % (self.container_name, len(self.listeners)))
...
class SSHSessionForDockerVolumeConchUser:
...
class SFTPServerForDockerVolumeConchUser:
    def __init__(self, avatar):
        self.avatar = avatar
...
class UnixSFTPDirectory:
    ...
    def __init__(self, server, directory):
        self.server = server
        if directory == "/":
            self.files = [x.split("/")[1]
                          for x
                          in self.server.avatar.volumes.keys()]
        elif any(x for x in self.server.avatar.volumes.keys() if x.startswith(directory)):
            self.files = [x.replace(directory, "").split("/")[1]
                          for x
                          in self.server.avatar.volumes.keys()
...
components.registerAdapter(SFTPServerForDockerVolumeConchUser, DockerVolumeConchUser, filetransfer.ISFTPServer)
components.registerAdapter(SSHSessionForDockerVolumeConchUser, DockerVolumeConchUser, session.ISession)
